...
import { getSupermarketProducts, SortOption } from '@/lib/api';
...
export default function SupermarketProducts() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['supermarketProducts', id, page, sortBy],
    queryFn: () => getSupermarketProducts(id!, page, 20, sortBy),
    enabled: !!id,
  });
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-6 py-8">
        <Link to="/supermarkets" className="inline-flex items-center text-sm text-muted-foreground hover:text-foreground mb-6">
          <ArrowLeft className="w-4 h-4 mr-1" />
          Terug naar supermarkten
        </Link>
